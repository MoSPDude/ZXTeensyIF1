...
static int fdd_isready(FLOPPY_DRIVE *fd)
{
	...
	if (!file)
	{
		file = SD.open(fdd->fdd_filename, FILE_READ);
		if (file)
		{
...
static fd_err_t fdd_load_track_header(DSK_FLOPPY_DRIVE *fdd, int head)
{
        ...
        if (fdd->fdd_fp->read(fdd->fdd_track_header, 256) < 256)
                ...
                int head,  fdc_byte *buf, int *len)
{
	...
	if (trklen > *len) 
        ...
        {
                if (fdd->fdd_fp->read(buf, trklen) < (size_t)(*len))
			err = FD_E_DATAERR;
        }
        return err;
}
			...
			int deleted, int skip_deleted, int mfm, int multi)
{
	...
	if (err == FD_E_DATAERR || err == 0)
	{
		...
		if (fdd->fdd_fp->write(buf, len) < (size_t)len)
			err = FD_E_READONLY;
		fdd->fdd_dirty = 1;
 ...
 */
	...
	{
		img_trklen = fdd->fdd_disk_header[0x32] + (256 * fdd->fdd_disk_header[0x33]);
 ...
 * 40 tracks that will grow to 80 tracks */
	fdd->fdd_fp->seek(trkoff, SeekSet);
	...
	fdd->fdd_fp->seek(0, SeekSet);
	if (fdd->fdd_fp->write(fdd->fdd_disk_header, 256) < 256)
	{
		memcpy(fdd->fdd_disk_header, oldhead, 256);
		return FD_E_READONLY;
	}
